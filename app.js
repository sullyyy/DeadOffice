...
function preload() {
  ...
  enemies.push(
    new Zombie(300, 200, 36, 70, loadImage("assets/img/zombie.png"), 2)
  );
  enemies.push(
    new Zombie(500, 500, 36, 70, loadImage("assets/img/zombie.png"), 3)
  );
}
  ...
  for (let i = 0; i < 10; i++) {
    for (let j = 0; j < 10; j++) {
      if (map_array_0[i][j] == 1) {
        ...
        map_array_0[i][j] = new VendingMachine(
          map_array_0[i][j],
          19,
          0,
          64,
      ...
      } else map_array_0[i][j] = new Tile(map_array_0[i][j], 0, 0, 100, 100);
    }
  }
}
...
function draw() {
  switch (gameState) {
    ...
    case GAME_OVER:
      ...
      image(
        dwight_dead,
        dwight.x + camera.offSetX,
        dwight.y + 70 + camera.offSetY,
        dwight_dead.width,
        dwight_dead.height
      );
      Menu.s_drawGameOver();
