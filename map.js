...
class Map {
	...
	drawZ()
	{
		...
		for(let k = 0; k < z_index_map.length; k++)
		{
			...
			if(z_index_map[k].j*100+camera.offSetX > -100 && z_index_map[k].i*100+camera.offSetY > -100 && z_index_map[k].j*100+camera.offSetX < camera.width + 100 && z_index_map[k].i*100+camera.offSetY < camera.height + 100)
			{
				//checks if background tile
				if(z_index_map[k].bckgrnd == true)
				{
					//checks if tile is positionable
					if(map_array[this.current_floor][z_index_map[k].i][z_index_map[k].j] instanceof PositionableTile)
						image(tilemap, z_index_map[k].j*100+camera.offSetX, z_index_map[k].i*100+camera.offSetY+map_array[this.current_floor][z_index_map[k].i][z_index_map[k].j].decalY, 100, 100,map_array[this.current_floor][z_index_map[k].i][z_index_map[k].j].id*100,0,100,100);
					else
						image(tilemap, z_index_map[k].j*100+camera.offSetX, z_index_map[k].i*100+camera.offSetY, 100, 100,map_array[this.current_floor][z_index_map[k].i][z_index_map[k].j].id*100,0,100,100);
				}
