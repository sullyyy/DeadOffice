...
class Character {
  ...
  checkCollision(x, y, x2, y2) {
    ...
    let x2w =
      x2 +
      map.map_array[map.current_floor][floor(y2 / 100)][floor(x2 / 100)]
        .hitboxW;
    let y1 = y + 60;
    let y1h = y + 70;
    let y2h =
    ...
    if (x1 > x2 && x1 < x2w && y1 > y2 && y1 < y2h) {
      if (
        map.map_array[map.current_floor][floor(y1 / 100)][floor(x1 / 100)].id !=
        1
      ) {
        colX = x2;
        colY = y2;
    ...
    if (x1w > x2 && x1w < x2w && y1 > y2 && y1 < y2h) {
      if (
        map.map_array[map.current_floor][floor(y1 / 100)][floor(x1w / 100)]
          .id != 1
      ) {
        colX = x2;
        colY = y2;
    ...
    if (x1 > x2 && x1 < x2w && y1h > y2 && y1h < y2h) {
      if (
        map.map_array[map.current_floor][floor(y1h / 100)][floor(x1 / 100)]
          .id != 1
      ) {
        colX = x2;
        colY = y2;
    ...
    if (x1w > x2 && x1w < x2w && y1h > y2 && y1h < y2h) {
      if (
        map.map_array[map.current_floor][floor(y1h / 100)][floor(x1w / 100)]
          .id != 1
      ) {
        colX = x2;
        colY = y2;
    ...
    for (let i = -1; i < 2; i++) {
      for (let j = -1; j < 2; j++) {
        ...
        let x2 =
          floor(x / 100) * 100 +
          i * 100 +
          map.map_array[map.current_floor][floor(y / 100) + j][
            floor(x / 100) + i
          ].hitboxX;
        let y2 =
          floor(y / 100) * 100 +
        ...
        if (this.checkCollision(x, y, x2, y2) == false) return false;
      }
    }
...
class Zombie extends Character {
  ...
  static s_drawChaseLine() {
    for (let i = 0; i < 2; i++) {
      if (enemies[i].zombieState == STATE.CHASING) enemies[i].drawChaseLine();
    }
  }
